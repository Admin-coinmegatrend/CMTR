...
#include "zcoinmegatrendcontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zcmtr ought to be enough for anybody." - Bill Gates, 2017
    ui->zcmtrpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzcmtrSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzcmtr->setEnabled(false);
        ui->pushButtonMintzcmtr->setToolTip(tr("zcmtr is currently disabled due to maintenance."));

        ui->pushButtonSpendzcmtr->setEnabled(false);
        ui->pushButtonSpendzcmtr->setToolTip(tr("zcmtr is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zcmtrpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzcmtr_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zcmtr is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zcmtr...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zcmtr in ") +
...
void PrivacyDialog::on_pushButtonSpendzcmtr_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zcmtr is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zcmtr
        sendzcmtr();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zcmtr
    sendzcmtr();
}

void PrivacyDialog::on_pushButtonZcoinmegatrendControl_clicked()
{
    ZcoinmegatrendControlDialog* zcmtrControl = new ZcoinmegatrendControlDialog(this);
    zcmtrControl->setModel(walletModel);
    zcmtrControl->exec();
}

void PrivacyDialog::setZcoinmegatrendControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzcmtrSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzcmtr()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid coinmegatrend Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zcmtrpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zcmtr is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " coinmegatrend </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zcmtr</b>";
    ...
    // use mints from zcmtr selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZcoinmegatrendControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZcoinmegatrendControlDialog::GetSelectedMints();
    }
    ...
    // Clear zcoinmegatrend selector in case it was used
    ZcoinmegatrendControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zcmtr Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " coinmegatrend, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zcmtr Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zcmtr </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zcmtr "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zcmtr "));
    ui->labelzcmtrAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzcmtrSyncStatus->setVisible(fShow);
}
