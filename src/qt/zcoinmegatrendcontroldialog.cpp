...
#include "ui_zcoinmegatrendcontroldialog.h"
...
std::list<std::string> ZcoinmegatrendControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZcoinmegatrendControlDialog::listMints;

ZcoinmegatrendControlDialog::ZcoinmegatrendControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZcoinmegatrendControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZcoinmegatrendControlDialog::~ZcoinmegatrendControlDialog()
{
    delete ui;
}

void ZcoinmegatrendControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZcoinmegatrendControlDialog::updateList()
{
...
void ZcoinmegatrendControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZcoinmegatrendControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZcoinmegatrend_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZcoinmegatrendControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZcoinmegatrendControlDialog::GetSelectedMints()
{
...
void ZcoinmegatrendControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
