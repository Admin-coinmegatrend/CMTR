...
// Copyright (c) 2019 The CoinMegaTrend developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#if defined(HAVE_CONFIG_H)
#include "config/coinmegatrend-config.h"
#endif
...
void PrepareShutdown()
{
    ...
    RenameThread("coinmegatrend-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "coinmegatrend.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "coinmegatrendd.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the coinmegatrend and zcmtr money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 16031, 5520));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in coinmegatrend/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in coinmegatrend/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, coinmegatrend, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in coinmegatrend/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all coinmegatrend specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:16031"));
    ...
    strUsage += HelpMessageOpt("-backupzcoinmegatrend=<n>", strprintf(_("Enable automatic wallet backups triggered after each zcmtr minting (0-1, default: %u)"), 1));

//    strUsage += "  -anonymizecoinmegatrendamount=<n>     " + strprintf(_("Keep N coinmegatrend anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) %i The CMTR Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    RenameThread("coinmegatrend-loadblk");
...
/** Sanity checks
 *  Ensure that coinmegatrend is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. coinmegatrend Core is shutting down."));
...
#endif
    // Make sure only a single coinmegatrend process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. coinmegatrend Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("coinmegatrend version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                // coinmegatrend: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() >= Params().Zerocoin_AccumulatorStartHeight()) {
                        RecalculateZcoinmegatrendMinted();
                        RecalculateZcoinmegatrendSpent();
                    }
                    RecalculatecoinmegatrendSupply(1);
                }
                ...
                // coinmegatrend: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of coinmegatrend Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart coinmegatrend Core to complete") << "\n";
        ...
        bool fEnableZcoinmegatrendBackups = GetBoolArg("-backupzcoinmegatrend", true);
        pwalletMain->setZcoinmegatrendAutoBackups(fEnableZcoinmegatrendBackups);
    }  // (!fDisableWallet)
#else  // ENABLE_WALLET
...
// XX42 Remove/refactor code below. Until then provide safe defaults
    nAnonymizecoinmegatrendAmount = 2;
...
//    }
//
//    nAnonymizecoinmegatrendAmount = GetArg("-anonymizecoinmegatrendamount", 0);
//    if (nAnonymizecoinmegatrendAmount > 999999) nAnonymizecoinmegatrendAmount = 999999;
//    if (nAnonymizecoinmegatrendAmount < 2) nAnonymizecoinmegatrendAmount = 2;
    ...
    LogPrintf("Anonymize coinmegatrend Amount %d\n", nAnonymizecoinmegatrendAmount);
    ...
    /* Denominations
       ...
       For example:
       1coinmegatrend+1000 == (.1coinmegatrend+100)*10
       10coinmegatrend+10000 == (1coinmegatrend+1000)*10
    */
